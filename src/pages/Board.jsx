...
export default function Board() {
  const { jobs, setJobs, resumes, addToast } = useApp()
  ...
  const handleDrop = (e, targetStatus) => {
    ...
    setJobs((prev) =>
      prev.map((j) => {
        ...
        if (j.status === targetStatus) return j
        ...
        return syncInterviewRounds({
          ...
          timeline: [...timeline, {
            ...
            action: '状态变更',
            detail: `从 ${j.status} 更新为 ${targetStatus}`,
          }],
        })
      })
    )
  ...
  const markAs = (job, newStatus, label, e) => {
    if (e) e.stopPropagation()
    setMenuJobId(null)
    setJobs((prev) =>
      prev.map((j) => {
  ...
  const saveFollowUp = () => {
    ...
    setJobs((prev) =>
      prev.map((j) => j.id === followUpJob.id ? { ...j, nextAction: followUpText } : j)
    )
